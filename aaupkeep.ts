...
async function revenueCalc() {
    ...
    { nations (id: ${data1.alliances.data[0].nations[i].id}, first: 500) 
        ...
        if ((data1.alliances.data[0].nations[i].vacation_mode_turns === 0) && (data1.alliances.data[0].nations[i].tax_id == 5496)) {

            console.log(data1.alliances.data[0].nations[i])

            const data = await request(endpoint, query1)
            ...
            aaFood += convert(((totalFood - (totalFoodCost - (soldiers / 9000)) * 7)))
    ...
    console.log([
        ...
        `Uranium: ${Math.sign(aaUranium) * Math.round(Math.abs(aaUranium) * 10) / 10}`,
        `Lead: ${Math.sign(aaLead) * Math.round(Math.abs(aaLead) * 10) / 10}`,
        `Iron: ${Math.sign(aaIron) * Math.round(Math.abs(aaIron) * 10) / 10}`,
        `Bauxite: ${Math.sign(aaBauxite) * Math.round(Math.abs(aaBauxite) * 10) / 10}`,
